...
| `assets/img/` | 이관 완료한 이미지 (`books/` 총서 표지, `main/` 배너) |
...
1. 모든 내부 링크가 `#` placeholder. 하위 페이지 제작 시 연결.
2. 검색 폼은 UI만 존재. 정적 사이트용 검색(예: 클라이언트 인덱스) 별도 구현 필요.
3. 통계 수치(총서 14권 / 박태준 연구총서 8권)는 구 사이트 목록 기준. 확정 필요.

## 다음 단계

- [ ] 메인 시안 확정 (레이아웃·색·타이포)
- [x] 메인 페이지 이미지 이관, 상대경로 전환 (2026-09-04)
